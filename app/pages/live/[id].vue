<template>
  <div class="flex h-dvh w-full flex-col bg-black lg:flex-row overflow-hidden font-sans">
    ...
    <div
      ...
      <template v-else-if="roomInfo">
        <div
          class="flex-none flex items-center justify-between border-b border-base-content/5 bg-base-100/90 p-3 lg:p-4 backdrop-blur-md z-10 shadow-sm">
          ...
          <div class="flex gap-2 items-center relative">
            <div class="relative flex-1 group">
              <input v-model="textContent" type="text" :placeholder="t('detail.say_something')"
                class="input input-bordered w-full rounded-full bg-base-200/50 pl-4 pr-10 focus:outline-none focus:bg-base-100 focus:border-primary transition-all text-sm"
                :disabled="isSending" @keydown.enter="handleSend" />
              <span v-if="textContent.length > 0"
                class="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-base-content/40 font-mono">
                {{ textContent.length }}
              </span>
            </div>

            <button
              class="btn btn-circle btn-primary shadow-lg shadow-primary/30 transition-transform active:scale-95 disabled:bg-base-200 disabled:text-base-content/20 disabled:shadow-none"
